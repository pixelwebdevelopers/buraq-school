        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-2 sm:p-4 backdrop-blur-sm print:static print:bg-white print:p-0 print:block">
            ...
            <div className="w-full max-w-5xl rounded-2xl bg-white shadow-2xl overflow-hidden max-h-[95vh] flex flex-col print:shadow-none print:max-h-none print:rounded-none">
                ...
                <div ref={componentRef} id="printable-student-detail" className="overflow-y-auto px-6 py-6 flex-1 bg-white print:overflow-visible print:px-2 print:py-0 print:text-[11px]">
                    ...
                    {/* Print Header (Visible only on print or specific style) */}
                    <div className="hidden print:flex mb-4 pb-2 border-b-2 border-gray-800 items-center justify-between">
                        <div className="flex items-center gap-3">
                            <img src="/logo.png" alt="Buraq School" className="w-[50px] h-[50px] object-contain grayscale" />
                            <div>
                                <h1 className="text-xl font-black text-gray-900 tracking-tight">BURAQ SCHOOL SYSTEM</h1>
                                <p className="text-gray-600 font-medium text-xs mt-0.5">Campus: {branchName || student.Branch?.name || 'N/A'}</p>
                                <p className="text-gray-500 text-[10px]">STUDENT ADMISSION RECORD</p>
                            </div>
                        </div>
                        <div className="text-right border-l-2 pl-3 border-gray-200">
                            <p className="text-xs text-gray-500 font-bold">Admission No.</p>
                            <p className="text-xl font-mono text-gray-900 leading-none mt-0.5">{student.admissionNo}</p>
                            <p className="text-[10px] text-gray-400 mt-0.5">Date: {new Date(student.createdAt).toLocaleDateString()}</p>
                        </div>
                    </div>
